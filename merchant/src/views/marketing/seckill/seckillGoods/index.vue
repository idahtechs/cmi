      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini" :row-class-name="tableRowClassName" @rowclick.stop="closeEdit">
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button v-if="tableFrom.type !== '5'" type="text" size="small" @click="onEditLabel(scope.row.product_id)">编辑标签</el-button>
  ...
  methods: {
    ...
    onEditLabel(id) {
      ...
      seckillDetailApi(id).then(async(res) => {
        ...
        if (res.data.mer_labels && res.data.mer_labels.length) {
          const arrNew = res.data.mer_labels.map(item => {
            return Number(item)
          })
          this.labelForm = {
            mer_labels: arrNew
