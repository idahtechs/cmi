				...
				<view class='item acea-row row-between-wrapper' v-for="(item,index) in collectProductList" :key="index" v-if="item.spu" @tap.stop="goDetail(item.spu)">
					<view class="left acea-row row-between-wrapper">
						...
						<view class='pictrue' >
							...
							<view v-if="item.spu.status !== 1" class="off">已下架</view>
						</view>
					</view>
...
<style scoped lang="scss">
	...
	::v-deep .collectionGoods .item .easy-loadimage, .collectionGoods .item uni-image{
		width: 100%;
		height: 100%;
		border-radius: 10rpx;
	}
...
.user_store_attention{
	...
	.store_recommend{
		...
		.list{
			width: 210rpx;
			.picture,::v-deep image,::v-deep .easy-loadimage,uni-image{
				width: 210rpx;
				height: 210rpx;
				border-radius: 10rpx;
				position: relative;
