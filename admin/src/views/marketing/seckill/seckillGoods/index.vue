  ...
  <div class="divBox">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        ...
        <el-button
          v-show="Number(tableFrom.type) < 3"
          size="mini"
      ...
      >
          ...
          <template slot-scope="scope">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <!--<el-form-item label="商品分类：">
                <template v-if="scope.row.merCateId.length">
                  <span
                    v-for="(item, index) in scope.row.merCateId"
                    :key="index"
                    class="mr10"
        ...
        <el-table-column prop="store_name" label="活动标题" min-width="180" />
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="onEditLabel(scope.row.product_id)">编辑标签</el-button>
            <el-button
      ...
      <div v-loading="loading" style="margin-top: 5px;">
        <div class="box-container">
          ...
          <div class="acea-row">
            ...
            <div class="list sp"><label class="name">商品名称：</label>{{ detailFormValidate.store_name }}</div>
    ...
    <el-dialog
      ...
      :before-close="handleClose"
    >
      <el-form ref="labelForm" :model="labelForm" @submit.native.prevent v-loading="labelLoading">
        <el-form-item>
  ...
  methods: {
    ...
    onEditLabel(id) {
      ...
      seckillDetailApi(id).then(async(res) => {
        ...
        if (res.data.sys_labels && res.data.sys_labels.length) {
          const arrNew = res.data.sys_labels.map(item => {
            return Number(item)
          })
          this.labelForm = {
            sys_labels: arrNew
